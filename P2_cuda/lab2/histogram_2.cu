...
__global__ void histogramSharedMemory(unsigned char* image, int* hist, size_t imageSize, int pixelsPerThread) {

	__shared__ int sharedHist[GRAY_LEVELS];

	int tid = threadIdx.x;
	int totalThreads = blockDim.x;

	for (int i = tid; i < GRAY_LEVELS; i += totalThreads) {
	...
	for (int i = 0; i < pixelsPerThread; i++) {
		...
		int pixelIdx = globalTid + i * gridSize;
...
int main(int argc, char** argv) {
	...
	histogramSharedMemory<<<blocksPerGrid, threadsPerBlock>>>(d_image, d_hist, imageSize, pixelsPerThread);
	cudaDeviceSynchronize();
	...
	int maxBlocksPerSM;
	cudaOccupancyMaxActiveBlocksPerMultiprocessor(&maxBlocksPerSM, histogramSharedMemory, threadsPerBlock, 0);
	cudaDeviceSynchronize();

...
int main() {
    ...
    dim3 blockSizes[] = {
        ...
        dim3(16,
             32) // 512 threads (half-maximum, might be better for occupancy)
    };
    ...
    for (dim3 blockSize : blockSizes) {
        ...
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            &maxBlocksPerSM, initMatrix, blockSize.x * blockSize.y, 0);

        float occupancy = (float)(maxBlocksPerSM * blockSize.x * blockSize.y) /
                          prop.maxThreadsPerMultiProcessor;
